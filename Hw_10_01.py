...
for line in fh:
    if 'From ' in line:             # Searches for 'From ' for each line through file
        ...
        counts[time_hr]=counts.get(time_hr,0)+1     # Creates new entries into counts using time_hr variable

for k,v in counts.items():          # breaks dic into tuples
    lst_hr.append((k,v))            # collect and stores k,v into tupples
srtd_lst_hr = sorted(lst_hr)        # sorts tuples by virtue of their keys

for k,v in srtd_lst_hr:             # for k,v in sorted list of tuples
    print(k,v)                      # print the k and v
